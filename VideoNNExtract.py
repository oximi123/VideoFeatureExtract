...
class P3D(nn.Module):
    ...
    def __init__(self, block, layers, modality='RGB',
        ...
        self.maxpool_2 = nn.MaxPool3d(kernel_size=(2, 1, 1), padding=0,
                                      stride=(2, 1, 1))  # pooling layer for res2, 3, 4. （2,1,1）图像维度不缩放，视频维度缩放
    ...
    def forward(self, x):
        ...
        x = self.bn1(x)
        x = self.relu(x)
        x = self.maxpool(x)  #图像维度缩放
        #视频维度缩放
        x = self.maxpool_2(self.layer1(x))  # Part Res2 视频帧数/2
        x = self.maxpool_2(self.layer2(x))  # Part Res3 视频帧数/2
        x = self.maxpool_2(self.layer3(x))  # Part Res4 视频帧数/2
...
if __name__ == '__main__':
    ...
    video = np.zeros((1,framenum, 160, 160, 3), dtype='float16')
    cnt = 0
    while (cap.isOpened()):
        a, b = cap.read()
        # cv2.imshow('%d' % cnt, b)
    ...
    video = torch.Tensor.view(video,[video.shape[0],video.shape[4],video.shape[1],video.shape[2],video.shape[3]])
    video = video.type(torch.FloatTensor)
    print(video.shape)
    model = P3D199(pretrained=True, modality='RGB', num_classes=400)
    model = model # 输入的帧数需要小于32才能最后变成一维特征，且必须大于16帧
    with torch.no_grad():
        data = torch.autograd.Variable(video)  # if modality=='Flow', please change the 2nd dimension 3==>2
        out = model(data)
        print(out.size(), out)
